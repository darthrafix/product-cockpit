...
import { authOptions, TOOL_PERMISSIONS, type Role } from '@/lib/auth';
...
const ALL_TOOLS = {
  ...
  ado_create_work_item: tool({
    description: 'Cria um work item no Azure DevOps. Requer perfil admin.',
    parameters: z.object({
  ...
  ado_update_work_item: tool({
    description: 'Atualiza título, descrição, estado ou tags de um work item. Requer perfil admin.',
    parameters: z.object({
      id: z.number(),
...
export async function POST(req: Request) {
  ...
  const role: Role = (session?.user as any)?.role ?? 'viewer';
  const allowed = TOOL_PERMISSIONS[role];

  // Filter tools to only those the role can use
  const tools = Object.fromEntries(
    Object.entries(ALL_TOOLS).filter(([k]) => allowed.includes(k))
  ) as typeof ALL_TOOLS;

  const { messages } = await req.json();

  const roleNote = `\n\n## Usuário atual\nEmail: ${session?.user?.email ?? 'desconhecido'} | Perfil: **${role}**\nFerramentas disponíveis para este perfil: ${allowed.join(', ')}`;

  const result = await streamText({
    model: anthropic('claude-sonnet-4-5'),
    system: SYSTEM_PROMPT + roleNote,
    messages,
    maxSteps: 10,
    tools,
  });
